...
<?
  ...
  @extract($_SESSION); 
   ...
   $num_match = mysql_num_rows($result); //있으면1, 없으면0
   ...
   if(!$num_match) //검색된 레코드가 없으면
   {
    ...
    else    //해당 아이디가 검색되었으면..
    {
      ...
      //$row[id], $row[pass], ... $row[level]
         ...
         $sql ="select * from member where id='$id' and pass=password('$pass')";  //아이디/패스워드를 검색
         $result = mysql_query($sql, $connect);
         $num_match = mysql_num_rows($result); //나오면 1, 없으면 0
